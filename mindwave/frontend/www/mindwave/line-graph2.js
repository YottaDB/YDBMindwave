 ...
 * [
 *   {
 *     date: UnixEpochInMilliSeconds,
 *     value: value
 *   },
 *   repeat structure above for each date/value
 * ]
...
function LineGraph(argsMap) {
   ...
   */
  this.slideData = function(updatedData) {
    // validate data
    ...
    validatedData.values.forEach(function(dataArrays, index) {
      ...
      dataArrays.forEach(function(value) {
        //console.log("slide => add new value: " + JSON.stringify(value));
        // push each new value onto the existing data array
        existingDataArrayForIndex.push(value);
        // shift the front value off to compensate for what we just added
        existingDataArrayForIndex.shift();
      })
    });

    // rewrite startTime and endTime into data to allow X axis scale to work
    ...
    data.endTime = (validatedData.endTime > data.endTime ? validatedData.endTime : data.endTime);

    // console.log("slide => add num new values: " + newValueCount);
    // console.log("old startDate: " + oldStartTime);
    // console.log("startDate: " + validatedData.startTime);
    // console.log("1st date value: " + new Date(data.values[0][0].date));
    // console.log("endDate: " + validatedData.endTime);
    // console.log("date diff: " + (data.values[0][0].date - oldStartTime.getTime()));
    // console.log("data:");
  ...
  var processDataMap = function(dataMap) {
    ...
    var dataValues = getRequiredVar(dataMap, 'values', "The data object must contain a 'values' value with a data array that contains the value and the date in milliseconds since unix epoch.")
    ...
    dataValues.forEach(function (value, index) {
      value.forEach(function (item) {
        ...
        if ((item.date < startTime) || (startTime === 0)) {
          startTime = item.date;
        }
        ...
        newDataValues[index] = value.slice(0);

        // get the maximum value
        if ((maxValues[index] < item.value) || (maxValues[index] === undefined)) {
    ...
    lineFunction = d3.svg.line()
      ...
      .y(function(d, i) { 
        ...
        */
        ...
        //debug("Line Y => data: " + d.value + " scale: " + _y)
